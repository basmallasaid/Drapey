  ...
  const itemRows = items
    .map(
      ...
      <tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #E9DED0;font-size:14px;color:#2A1D17;text-align:right;">$${Number(item.unit_price).toFixed(2)}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #E9DED0;font-size:14px;color:#2A1D17;text-align:right;font-weight:600;">$${Number(item.total_price).toFixed(2)}</td>
      </tr>`
    )
    .join("");
            ...
            <td style="padding:0 32px 32px;">
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#F7F1E8;border-radius:6px;padding:16px;">
                <tr>
                  ...
                  <td style="padding:6px 16px;font-size:14px;color:#2A1D17;text-align:right;font-weight:500;">$${Number(order.subtotal).toFixed(2)}</td>
                </tr>
                <tr>
                  <td style="padding:6px 16px;font-size:14px;color:#765442;">Shipping</td>
                  <td style="padding:6px 16px;font-size:14px;color:#2A1D17;text-align:right;font-weight:500;">${Number(order.shipping_fee) === 0 ? "FREE" : "$" + Number(order.shipping_fee).toFixed(2)}</td>
                </tr>
                ...
                <tr>
                  ...
                  <td style="padding:10px 16px 6px;font-size:18px;color:#047857;text-align:right;font-weight:700;border-top:2px solid #E9DED0;">$${Number(order.total_amount).toFixed(2)}</td>
                </tr>
              </table>
            </td>
          </tr>
            ...
            <td style="padding:0 32px 24px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td style="padding:6px 0;font-size:14px;color:#2A1D17;font-weight:500;">$${Number(order.total_amount).toFixed(2)}</td>
                </tr>
              </table>
            </td>
          </tr>
...
export async function sendOrderCancellationNotification(order, items) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: emailFrom,
      to: adminEmails,
      subject: `Order Cancelled ${orderId} â€” $${Number(order.total_amount).toFixed(2)}`,
...
export async function sendOrderNotification(order, items) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: emailFrom,
      to: adminEmails,
      subject: `New Order ${orderId} â€” $${Number(order.total_amount).toFixed(2)}`,
