...
import { useFav, useCart, useAuth } from '../../providers';
      ...
      >
        ...
        <div className="aspect-[3/4] relative overflow-hidden bg-white shadow-sm">
          ...
          <div className={`absolute bottom-4 left-0 right-0 px-4 flex flex-col gap-2 transition-all duration-500 ${
            isHovered ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
             ...
             <button
              ...
              disabled={isAdmin}
              className="bg-white text-dark-brown py-3 text-[10px] font-bold uppercase tracking-[2px] shadow-xl hover:bg-dark-brown hover:text-white transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white disabled:hover:text-dark-brown"
            >
              Add to Cart
            </button>
          </div>
          ...
          <button
            ...
            disabled={isAdmin}
            className="absolute top-4 right-4 p-2.5 bg-white/80 backdrop-blur-md rounded-full shadow-sm text-dark-brown hover:text-tan transition-all disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:text-dark-brown"
          >
