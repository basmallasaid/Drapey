...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF8F5] text-[#3E3A36]">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#EBE2DA] transform transition-transform duration-300 ease-in-out lg:translate-x-0 flex flex-col ${
      ...
      >
        <div className="p-8 pb-4 flex items-center justify-between">
          ...
          <button
            onClick={closeSidebar}
            className="lg:hidden p-2 text-[#8E8A84] hover:text-[#3E3A36] rounded-lg ml-2"
      ...
      <div className="flex-1 lg:pl-64 min-w-0">
        <header className="h-20 bg-white border-b border-[#EBE2DA] flex items-center justify-between px-4 sm:px-8 gap-4 sticky top-0 z-30">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2 text-[#8E8A84] hover:text-[#3E3A36] rounded-lg -ml-1"
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] sm:text-xs text-[#8E8A84] truncate">Welcome back, {adminName}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-6">
            <div className="relative hidden md:block">
              ...
              <input 
                type="text" 
                placeholder="Search..." 
                className="bg-[#FAF8F5] border-none rounded-full py-2 pl-10 pr-4 text-sm w-40 lg:w-64 focus:ring-1 focus:ring-[#EBE2DA] outline-none" 
              />
            </div>
            <button className="p-2 text-[#8E8A84] hover:text-[#3E3A36] -mr-2 md:mr-0" aria-label="Notifications">
              <Bell size={20} />
            </button>
            <div className="flex items-center gap-3 pl-2 sm:pl-6 border-l border-[#EBE2DA] min-w-0">
              ...
              <div className="w-9 h-9 sm:w-10 sm:h-10 bg-[#F3EFEA] rounded-full flex items-center justify-center font-bold text-[#3E3A36] shrink-0">{adminName.charAt(0).toUpperCase()}</div>
            </div>
          </div>
        </header>
