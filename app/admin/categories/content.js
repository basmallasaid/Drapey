  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[var(--color-dark-brown)]">Categories</h2>
          <p className="text-sm text-[var(--color-medium-brown)]">Organize your store collection into elegant groups</p>
        </div>
        <button
          onClick={startCreate}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {categoryList.length === 0 ? (
          <div className="col-span-full py-20 text-center bg-white rounded-[24px] border border-[var(--color-light-beige)]">
            <Folder size={48} className="mx-auto text-[var(--color-light-beige)] mb-4" />
            <p className="text-[var(--color-medium-brown)]">No categories found. Start by adding one.</p>
          </div>
        ) : (
          categoryList.map((c) => (
            <div key={c.id} className="group relative bg-white rounded-[24px] border border-[var(--color-light-beige)] p-6 hover:shadow-xl transition-all duration-300">
              <div className="flex items-center justify-between mb-6">
                <div className="w-16 h-16 rounded-2xl bg-[var(--color-light-beige)] overflow-hidden border border-[var(--color-light-beige)]">
                  {c.image_url ? (
                    <img src={c.image_url} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[var(--color-medium-brown)]"><ImageIcon size={20}/></div>
                  )}
                </div>
                <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                  <button onClick={() => startEdit(c)} className="p-2 text-[var(--color-dark-brown)] hover:bg-[var(--color-cream)] rounded-full transition-colors"><Edit2 size={16}/></button>
                  <button
              ...
              <div>
                <h4 className="text-lg font-bold text-[var(--color-dark-brown)]">{c.name}</h4>
                <div className="flex items-center gap-1.5 mt-1 text-[var(--color-medium-brown)]">
                  <LinkIcon size={12}/>
              ...
              {c.description && (
                <p className="mt-4 text-xs text-[var(--color-medium-brown)] line-clamp-2 leading-relaxed italic border-l-2 border-[var(--color-light-beige)] pl-3">
                  {c.description}
                </p>
              )}

              <div className="mt-6 pt-6 border-t border-[var(--color-cream)] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <LayoutGrid size={14} className="text-[var(--color-medium-brown)]" />
                  <span className="text-xs font-bold text-[var(--color-dark-brown)]">{c.products?.[0]?.count || 0} Products</span>
                </div>
                <div className="px-3 py-1 bg-[var(--color-cream)] rounded-full text-[10px] font-bold text-[var(--color-medium-brown)] uppercase">
                  Collection
                </div>
              </div>
            </div>
